...
MainComponent::MainComponent()
{
    ...
    auto mainHandler = eventSystem.add<SimpleEvent>(
        [](SimpleEvent event) { DBG("Main Handler : " + event.data); });

    eventSystem.send(SimpleEvent {"Event One"});
    {
        auto scopedHandler = eventSystem.add<SimpleEvent>(
            [](SimpleEvent event) { DBG("Scoped handler : " + event.data); });
    ...
    keyHandler = eventSystem.add<KeyEvent>(
        [](KeyEvent event)
        {
            if (event.isDown)
            {
